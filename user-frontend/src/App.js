...
class App extends React.Component {
  ...
  componentDidMount() {
    axios.get("http://localhost:8082/api/v1/")
    .then((res) => {
        this.setState({
            users:res.data,
            id:0,
  ...
  submit(event, id) {
    ...
    if(id === 0) {
      axios.post("http://localhost:8082/api/v1/", {
      ...
      axios.put("http://localhost:8082/api/v1/", {
        id: this.state.id,
        name: this.state.name,
        email: this.state.email, 
        password: this.state.password
  ...
  delete(id) {
    ...
    axios.delete(`http://localhost:8082/api/v1/${id}`)
    .then(()=> {
      this.componentDidMount();
    })
  }
  ...
  edit(id) {
    axios.get(`http://localhost:8082/api/v1/${id}`)
    .then((res) => {
